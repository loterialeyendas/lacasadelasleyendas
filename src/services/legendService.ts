...
export const parseQRData = (rawText: string): Legend | undefined => {
  ...
  // Si viene en formato URL (ej: https://.../?legend=sombreron o /reto/sombreron)
  try {
    if (clean.includes('http://') || clean.includes('https://') || clean.includes('?')) {
      ...
      const legendParam = url.searchParams.get('legend') || url.searchParams.get('id') || url.searchParams.get('code');
      if (legendParam) {
